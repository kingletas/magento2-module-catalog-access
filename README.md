...
None of these are hard. That is exactly why they are worth a module: each one is easy enough to write inline in four lines, and each of those four-line versions has the same defect as the last one. The repository throws where the caller expected null. The loop loads one product per iteration. The store-scoped query returns nothing because the store row does not exist. `trim()` gets a null. The emulation never gets stopped because the callback threw.
...
| Joining catalogue tables on `entity_id` | On Adobe Commerce an entity is one row per scheduled update and its values hang off `row_id`. The query does not fail — it joins the wrong rows, and returns next month's data today | `StagedEntityFilter` supplies the link field and narrows every query to the version that is live now, and is a no-op on Open Source |
...
| A product collection in the frontend area | `Magento\CatalogInventory\Model\AddStockStatusToCollection` is a `beforeLoad` plugin registered **only** there. The same code returns everything in cron and silently drops out-of-stock products on a storefront request | The loader sets `has_stock_status_filter`, so the answer does not depend on which area asked |
...
After saving a product, drop it from the memo — otherwise the value you just wrote is not the one you read back:
...
For another entity's attributes, give the resolver a different entity type — it is a `di.xml` argument, not a constant:
...
- **The memos are wired in `di.xml`, and that wiring is not tuning.** The container shares one instance per type, so injecting `RequestMemo` directly would give every resolver the same memo: one key namespace, one size bound, and a product-heavy loop evicting the category names it is about to ask for again. Each consumer has its own `virtualType`.
...
- **Nothing is written to the persistent cache.** One batched query per request per store already removes the cost that matters, and a cached category name is a name that has to be invalidated on rename, on move, on store-view change and on scheduled-update go-live. Four contracts to get right in exchange for microseconds. If your store wants one, replace the `<preference>` — that is what the interface is for. The version that goes wrong quietly is a cache with no tag and no lifetime: a category renamed in the admin never reaches the storefront until somebody flushes it by hand.
- **`loadBySkus()` does not filter by website.** `addStoreFilter()` would turn "load these products" into "load these products, if this store happens to sell them", and the call site cannot tell that result from a deleted SKU.
...
- **`StoreScope::getCurrentStoreId()` answers `0` when there is no current store.** In a CLI process that is the honest answer, and it is what `getStore()` would have thrown about.
